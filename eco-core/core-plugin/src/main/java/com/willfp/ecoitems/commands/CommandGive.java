...
public class CommandGive extends Subcommand {
    ...
    public CommandGive(@NotNull final EcoPlugin plugin) {
        super(plugin, "give", "ecoitems.command.give", false);
        reload();
    }
    ...
    public static void reload() {
        ITEM_NAMES.clear();
        ITEM_NAMES.addAll(EcoItems.values().stream().map(CustomItem::getKey).map(NamespacedKey::getKey).collect(Collectors.toList()));
    }
    ...
    public CommandHandler getHandler() {
        return (sender, args) -> {
            ...
            CustomItem item = EcoItems.getByName(itemID.toLowerCase());
            ...
            message = message.replace("%item%", item.getItem().getItemMeta().getDisplayName()).replace("%recipient%", reciever.getName());
            ...
            ItemStack toGive = item.getItem();
            toGive.setAmount(amount);
            reciever.getInventory().addItem(toGive);
